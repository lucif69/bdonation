...
import { trackRequest, resetTrack, donate, cancelDonation } from '../actions/blood';


const ActiveItem = ({ match, resetTrack, trackRequest, request, auth, donate, history, cancelDonation }) => {
        ...
        if (request !== null) {
            ...
            return (
                <div>
                    ...
                    <div className="row">
                        <div className="col s3"><strong>Patient Address</strong>: </div>
                        <div className="col s9">{address && address.city}, {address && address.state}, {address && address.country}</div>
                    </div>
                    <hr />
                    <div className="row">
                        <div className="col s3"><strong>Status</strong>: </div>
    ...
    return (
        <div>
            ...
            {auth.isAuthenticated && request && !request.status && <button onClick={onDonate} className="btn black">
                donate
            </button>}
            {auth.isAuthenticated && request && request.status && request.user === auth.user._id && <button onClick={onCancelDonate} className="btn black">
                cancel
            </button>}
...
export default connect(mapStateToProps, { trackRequest, resetTrack, donate, cancelDonation })(withRouter(ActiveItem));
